...
export async function listProducts(identity: Identity, query: PaginationQuery) {
  ...
  // Aggregate real on-hand stock and serial numbers per product
  const productIds = rawProducts.map((p) => p._id);

  const [lotSums, serialGroups] = await Promise.all([
    col(db, 'stockLots')
      .aggregate([
        {$match: {tenantId: identity.tenantId, productId: {$in: productIds}, quantityRemaining: {$gt: 0}}},
        {$group: {_id: '$productId', totalStock: {$sum: '$quantityRemaining'}}},
      ])
      .toArray(),
    col(db, 'serialUnits')
      .aggregate([
        {$match: {tenantId: identity.tenantId, productId: {$in: productIds}, status: 'InStock'}},
        {$group: {_id: '$productId', serials: {$push: '$serialOriginal'}}},
      ])
      .toArray(),
  ]);

  const stockMap = new Map(lotSums.map((s) => [s._id, s.totalStock]));
  const serialMap = new Map(serialGroups.map((g) => [g._id, g.serials]));

  const records = rawProducts.map((p) => ({
    ...p,
...
export async function getProductById(identity: Identity, id: string) {
  ...
  const [lots, serials, movements] = await Promise.all([
    col(db, 'stockLots')
      .find({tenantId: identity.tenantId, productId: id, quantityRemaining: {$gt: 0}})
      .toArray(),
    col(db, 'serialUnits')
      .find({tenantId: identity.tenantId, productId: id, status: 'InStock'})
      .toArray(),
    col(db, 'stockMovements')
      .find({tenantId: identity.tenantId, productId: id})
      .sort({date: -1, createdAt: -1})
      .limit(50)
      .toArray(),
  ]);

  const stock = lots.reduce((sum, l) => sum + (l.quantityRemaining || 0), 0);

  return {
    ...product,
    stock,
    serials: serials.map((s) => s.serialOriginal),
    movements,
    lots,
  };
}
...
export async function listServices(identity: Identity, query?: PaginationQuery) {
  ...
  if (query?.q) {
    ...
    filter.$or = [{name: qRegex}, {category: qRegex}, {description: qRegex}];
  }
...
export async function importDemoMasterData(identity: Identity, options?: { cutoffDate?: string }) {
  ...
  try {
    return await session.withTransaction(async () => {
      ...
      await col(db, 'suppliers').insertMany([
        {
          _id: sup1,
          tenantId: identity.tenantId,
          name: 'Supreme Computers India Pvt Ltd',
          phone: '+91 44 2841 2300',
          phoneNormalized: normalizePhone('+91 44 2841 2300'),
          email: 'orders@supremecomputers.in',
          address: 'Mount Road, Chennai',
          gst: '33AABCS1429B1Z8',
          gstNormalized: normalizeGstin('33AABCS1429B1Z8'),
          terms: 30,
          status: 'Active',
          createdAt: new Date(),
          createdBy: identity.userId,
      ...
      await col(db, 'customers').insertMany([
        {
          ...
          gst: '33AAACA1234A1Z5',
          gstNormalized: normalizeGstin('33AAACA1234A1Z5'),
          type: 'Business',
      ...
      await col(db, 'products').insertMany([
        {
          ...
          brand: 'HP',
          condition: 'New',
          model: '15-eg2009TU · Natural Silver',
          hsn: '84713010',
          costPaise: 5200000,
          sellingPricePaise: 6150000,
      ...
      await col(db, 'serviceCatalog').insertMany([
        {
          ...
          nameNormalized: 'laptop chip-level service',
          category: 'Chip-Level',
          description: 'Motherboard diagnositics, component-level power repair and BIOS programming.',
          ratePaise: 180000,
          taxBasisPoints: 1800,
          sac: '998713',
          warranty: 3,
        ...
        },
        {
          _id: uid('SRV'),
          tenantId: identity.tenantId,
          name: 'Windows OS Reinstall & Optimisation',
          nameNormalized: 'windows os reinstall & optimisation',
          category: 'Software',
          description: 'Clean OS install, drivers, critical updates and performance tuning.',
          ratePaise: 50000,
          taxBasisPoints: 1800,
          sac: '998713',
          warranty: 1,
      ...
      const serialSample = `DEMO-HP-${Date.now().toString().slice(-4)}`;

...
export async function initializeAccountBalances(db: Db, tenantId: string, session: ClientSession) {
  const opening = await db.collection<any>('openingSetups').findOne({tenantId, status: 'Finalized'}, {session});
  if (!opening) throw new AppError(409, 'Finalize opening setup before initializing Cash and Bank.');
  ...
  for (const account of accounts) {
    const expectedOpening = account === 'Cash' ? opening.openingCashPaise : opening.openingBankPaise;

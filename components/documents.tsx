...
export function DocumentComposer({
  ...
  function saveSales(receive = false) {
    ...
    const ok = run((s) => {
      ...
      return next;
    }, quotation ? 'Quotation saved.' : 'Invoice, stock, payment, outstanding and warranty updated.');
    if (ok) router.push(`${quotation ? '/quotations' : '/sales'}/${id}`);
  }

  return (
    <>
      <Link className="back-link" href={purchase ? '/purchases' : quotation ? '/quotations' : '/sales'}>
        <ArrowLeft size={14} />
        Back to {purchase ? 'purchases' : quotation ? 'quotations' : 'invoices'}
      </Link>
      ...
      <div className="stack">
        ...
        >
          <div className="form-body form-grid">
            <Field label={purchase ? 'Supplier *' : 'Customer *'}>
              {purchase && isLive && <input value={supplierSearch} onChange={(e) => setSupplierSearch(e.target.value)} placeholder="Search live suppliers…" />}
              {!purchase && isLive && <input value={customerSearch} onChange={(e) => setCustomerSearch(e.target.value)} placeholder="Search live customers…" />}
              <select
                value={customerId}
                onChange={(e) => {
                  setCustomerId(e.target.value);
                  const c = purchase
                    ? supplierChoices.find((c) => c.id === e.target.value) || state.suppliers.find((c) => c.id === e.target.value)
                    : customerChoices.find((c) => c.id === e.target.value) || state.customers.find((c) => c.id === e.target.value);
                  const days = purchase
                    ? (c as any)?.terms
              ...
              >
                ...
                {(purchase ? supplierChoices : (isLive ? customerChoices : state.customers)).map((c) => (
        ...
        >
                ...
                {(purchase && isLive ? productChoices : state.products)
                  ...
                  .map((p) => (
                    <option value={p.id} key={p.id}>
                      {p.name} · {money(purchase ? p.cost : p.price)} · {p.stock} on hand
                    </option>
                  ))}
              </select>
                  ...
                  <tr key={(l as any).clientLineKey || i}>
                    <td>
                      ...
                      <div className="line-meta">
                        ...
                        />
                        {l.productId && !service && l.lineType !== 'Charge' && (
                          <button
                            type="button"
                            onClick={() => {
                              if (!purchase && !quotation && isLive) {
                                setAllocatingLineIndex(i);
                              } else {
                                setSerialIndex(i);
                                setSerialText(l.serials.join('\n'));
                              }
                            }}
                          >
                            {purchase
          ...
          {!lines.length && (
            <Empty
              ...
              text={service ? 'Add the service work and charges below.' : 'Choose a product above to start this document.'}
            />
          )}
        ...
        <div className="form-actions">
          <Link className="btn secondary" href={purchase ? '/purchases' : quotation ? '/quotations' : '/sales'}>
            Cancel
          </Link>
          {purchase ? (
            isReceiptMode ? (
      ...
      {serialIndex !== null && (
        <Modal
          title={`Serial numbers · ${lines[serialIndex].name}`}
          onClose={() => setSerialIndex(null)}
        >
          <div className="form-body">
            {purchase ? (
              <Field label={`Enter ${lines[serialIndex].qty} serial number(s), one per line`}>
                <textarea value={serialText} onChange={(e) => setSerialText(e.target.value)} />
              </Field>
            ) : (
              <>
                <p className="notice">
                  Select {lines[serialIndex].qty} unit(s). Serials held for this customer are available to their sale.
                </p>
                <div className="serial-list">
                  {state.products
                    .find((p) => p.id === lines[serialIndex].productId)
                    ?.serials.map((n) => {
                      const blocked = state.reservations.some(
                        (r) => activeReservation(r) && r.customerId !== customerId && r.serials.includes(n)
                      );
                      return (
                        <label key={n}>
                          <input
                            type="checkbox"
                            disabled={blocked}
                            checked={lines[serialIndex].serials.includes(n)}
                            onChange={(e) =>
                              update(
                                serialIndex,
                                'serials',
                                e.target.checked
                                  ? [...lines[serialIndex].serials, n]
                                  : lines[serialIndex].serials.filter((x) => x !== n)
                              )
                            }
                          />
                          {n}
                          {blocked && <Badge>Reserved</Badge>}
                        </label>
                      );
                    })}
                </div>
                {!state.products.find((p) => p.id === lines[serialIndex].productId)?.serials.length && (
                  <p>This product uses quantity tracking; no serial selection is needed.</p>
                )}
              </>
            )}
          </div>
...
function LivePurchaseLifecycle({
  record,
  onRefresh,
}: {
  record: Purchase;
  ...
  const [closeRemainderOpen, setCloseRemainderOpen] = useState(false);
  const [reasonText, setReasonText] = useState('');
  const [busy, setBusy] = useState(false);
  const [invoiceNumber, setInvoiceNumber] = useState(record.reference || '');
  const [invoiceDate, setInvoiceDate] = useState(TODAY);

  const canConfirm = record.documentStatus === 'Draft';
  const canPost = record.billStatus === 'NotPosted' && record.documentStatus !== 'Cancelled';
  const canReceive =
    record.billStatus === 'Posted' &&
    !['Received', 'ClosedPartlyReceived'].includes(record.receiptStatus || '');
  const canCloseRemainder = record.receiptStatus === 'PartlyReceived';
  const canCancel =
    record.billStatus === 'NotPosted' &&
    record.receiptStatus === 'NotReceived' &&
    ...
    <>
      {canConfirm && (
        <Btn
          ...
          onClick={async () => {
            setBusy(true);
            const res = await confirmPurchaseOrderApi(record.id, record.version);
            setBusy(false);
            if (res.success) {
              notify('Purchase order confirmed.');
              onRefresh?.();
            } else {
              notify(res.error || 'Failed to confirm purchase order.');
            }
          }}
        >
          Confirm order
      ...
      {canCloseRemainder && (
        <Btn secondary disabled={busy} onClick={() => setCloseRemainderOpen(true)}>
          Close remainder
        </Btn>
      )}

      {canCancel && (
        <Btn secondary disabled={busy} onClick={() => setCancelOpen(true)}>
          Cancel order
        </Btn>
      )}
        ...
        <Modal title="Post supplier bill" onClose={() => setPostOpen(false)}>
          <form
            onSubmit={async (event) => {
              ...
              setBusy(true);
              const res = await postPurchaseBillApi(
                record.id,
                invoiceNumber.trim(),
                invoiceDate,
                record.version
          ...
          >
            ...
            <div className="form-actions">
              <Btn secondary onClick={() => setPostOpen(false)}>
                Cancel
              </Btn>
              <Btn type="submit" disabled={busy}>
                {busy ? 'Posting…' : 'Post bill'}
      ...
      {closeRemainderOpen && (
        <Modal title="Close unreceived remainder" onClose={() => setCloseRemainderOpen(false)}>
          <div className="form-body">
            <p>
              Are you sure you want to close the unreceived remainder of this purchase order? Received stock stays
              available, but remaining quantities cannot be received later.
            </p>
            <Field label="Reason (optional)">
              <input
                value={reasonText}
                onChange={(e) => setReasonText(e.target.value)}
                placeholder="Supplier out of stock, cancelled remaining..."
              />
            </Field>
          </div>
          <div className="form-actions">
            <Btn secondary onClick={() => setCloseRemainderOpen(false)}>
              Back
            </Btn>
            <Btn
              danger
              ...
              onClick={async () => {
                setBusy(true);
                const ok = await closePurchaseRemainderApi(record.id, record.version, reasonText.trim() || undefined);
                setBusy(false);
                if (ok) {
      ...
      {cancelOpen && (
        <Modal title="Cancel purchase order" onClose={() => setCancelOpen(false)}>
          <div className="form-body">
            <p>
              Cancel this unposted purchase order? It has no stock movements or financial effect, and will remain in
              history as cancelled.
            </p>
          </div>
          <div className="form-actions">
            <Btn secondary onClick={() => setCancelOpen(false)}>
              Keep order
            </Btn>
            <Btn
              danger
              ...
              onClick={async () => {
                setBusy(true);
                const ok = await cancelPurchaseApi(record.id, record.version);
                setBusy(false);
                if (ok) {
                  setCancelOpen(false);
                  onRefresh?.();
  ...
  return (
    <>
            ...
            <>
              ...
              {quotation && record.status !== 'Converted' && record.status !== 'Cancelled' && (
                <>
                  ...
                  <Btn secondary onClick={() => setQuoteCancelOpen(true)}>
                    Cancel quotation
                  </Btn>
                </>
              )}
              {quotation && record.status === 'Cancelled' && (
                <Btn secondary onClick={() => setQuoteReopenOpen(true)}>
                  Reopen quotation
                </Btn>
              )}
              {purchase && isLive && (
                <LivePurchaseLifecycle record={record as Purchase} onRefresh={refreshDetail} />
              )}
              ...
              {!purchase && !quotation && record.status !== 'Draft' && (
                <>
                  ...
                  <Link className="btn secondary" href={`/returns?reference=${id}`}>
                    Record return
                  </Link>
                </>
              )}
              {purchase && !quotation && balance(state, record) > 0 && (
                <Btn onClick={() => setPayment(true)}>
                  Pay supplier
                </Btn>
              )}
            </>
      ...
      {record ? (
        <>
          ...
          <div className="detail-grid spaced screen-only">
            <Card title={quotation ? 'Quotation actions' : 'Payment history'}>
              {quotation ? (
                <div className="body-pad actions">
                  <Link className="btn secondary" href={`/quotations/new?from=${record.id}`}>
                    Duplicate quotation
                  </Link>
                  <Link
                    className="btn secondary"
                    href={`/communication?customer=${(record as Bill).customerId}`}
                  >
                    Preview message
                  </Link>
                  {!['Converted', 'Cancelled'].includes(record.status) && (
                    <Btn secondary onClick={() => setCancel(true)}>
                      Cancel quotation
                    </Btn>
                  )}
                </div>
                ...
                <>
                  ...
                  {!state.payments.some((p) => p.reference === record.id) && (
                    <Empty
                      ...
                      text="Record the payment when money is actually received or paid."
                    />
                  )}
                </>
              )}
            ...
            <Card title="Related actions">
              <div className="body-pad stack">
                {!purchase && !quotation && (
                  <>
                    <h3>Supplier payment reference</h3>
                    <SupplyReference bill={record as Bill} state={state} />
                  </>
                )}
                {!purchase && !quotation && (
                  <Link className="text-link" href="/warranty">
                    Add coverage, photos or manage warranty
                  </Link>
                )}
                <Link
                  className="text-link"
                  href={`${purchase ? '/suppliers' : '/customers'}/${customer?.id}`}
                >
                  Open {purchase ? 'supplier' : 'customer'} profile <ArrowUpRight size={15} />
                </Link>
                {!quotation && (
                  <Link className="text-link" href={`/returns?reference=${record.id}`}>
                    Record {purchase ? 'supplier' : 'customer'} return <ArrowUpRight size={15} />
                  </Link>
                )}
              </div>
            </Card>
      ...
      {payment && record && (
        purchase ? (
          <SupplierSettlement
            supplierId={(record as Purchase).supplierId}
            defaultPurchaseId={(record as Purchase).id}
            onClose={() => {
              setPayment(false);
              refreshDetail();
            }}
      ...
      {quoteCancelOpen && record && (
        <QuotationCancelModal
          isOpen={quoteCancelOpen}
      ...
      {quoteReopenOpen && record && (
        <QuotationReopenModal
          isOpen={quoteReopenOpen}
          quotationId={record.id}
          version={detailData?.raw?.version || 1}
          onClose={() => setQuoteReopenOpen(false)}
          onSuccess={() => {
            setQuoteReopenOpen(false);
            notify('Quotation reopened to Draft.');
            refreshDetail();
          }}
        />
      )}

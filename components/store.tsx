...
type Store = {
  ...
  fetchInventoryLotsApi: (query?: any) => Promise<any>;
  fetchInventorySerialsApi: (query?: any) => Promise<any>;
  fetchInventoryMovementsApi: (query?: any) => Promise<any>;
...
export function StoreProvider({children}: {children: ReactNode}) {
  ...
  function run(fn: (s: State) => State, message: string) {
    if (isLive) {
      notify('This action is disabled in Live Mode until transaction modules are activated in Phases 3–5.');
      return false;
    }
    try {
      setState(fn(state));
  ...
  const refreshMasterData = useCallback(async () => {
    try {
      ...
      // Hydrate Phase 3 purchases and payments
      try {
        const [purRes, payRes] = await Promise.all([
          fetch('/api/purchases?limit=50'),
          fetch('/api/purchases/payments?limit=50'),
        ]);
        if (purRes.ok) {
          const purData = await purRes.json();
          setState((prev) => ({
            ...prev,
            purchases: (purData.records || []).map(mapPurchaseFromApi),
          }));
        }
        if (payRes.ok) {
          ...
          setState((prev) => ({
            ...prev,
            payments: (payData.records || []).map(mapPaymentFromApi),
          }));
        }
      } catch (e) {
        console.warn('Could not hydrate purchases/payments:', e);
      }
    } catch (err) {
  ...
  function resetDemoData() {
    if (typeof window !== 'undefined') {
      try {
        localStorage.removeItem('itech_demo_state_v1');
      } catch {}
    }
    setState(structuredClone(seed));
  ...
  async function fetchInventoryLotsApi(query: any = {}) {
    const p = new URLSearchParams();
    if (query.page) p.set('page', String(query.page));
    if (query.limit) p.set('limit', String(query.limit));
    if (query.productId) p.set('productId', query.productId);
    if (query.search) p.set('search', query.search);
    const res = await fetch(`/api/inventory/lots?${p.toString()}`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Failed to load stock lots.');
    return data;
  }

  async function fetchInventorySerialsApi(query: any = {}) {
    const p = new URLSearchParams();
    if (query.page) p.set('page', String(query.page));
    if (query.limit) p.set('limit', String(query.limit));
    if (query.productId) p.set('productId', query.productId);
    if (query.lotId) p.set('lotId', query.lotId);
    if (query.status) p.set('status', query.status);
    if (query.search) p.set('search', query.search);
    const res = await fetch(`/api/inventory/serials?${p.toString()}`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Failed to load serials.');
    return data;
  }

  async function fetchInventoryMovementsApi(query: any = {}) {
    const p = new URLSearchParams();
    if (query.page) p.set('page', String(query.page));
    if (query.limit) p.set('limit', String(query.limit));
    if (query.productId) p.set('productId', query.productId);
    ...
    const res = await fetch(`/api/inventory/movements?${p.toString()}`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Failed to load stock movements.');
    return data;
  }
